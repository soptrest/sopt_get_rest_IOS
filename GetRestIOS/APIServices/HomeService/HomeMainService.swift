...
//        guard let token: String = jwt as? String else { return }
    ...
    func getDetailTableData(date: String, completion: @escaping (NetworkResult<Any>) -> Void) {
        ...
        let data: Parameters = [ "date " : "2019/3" ]
        
        Alamofire.request(URL, method: .get, parameters: data, encoding: URLEncoding.httpBody, headers: header)
            .responseData { response in
                switch response.result {
                case .success:
                    if let value = response.result.value {
    ...
    func update(date: String, completion: @escaping (NetworkResult<Any>) -> Void) {
        let URL = APIConstants.HomeURL + "/portfolio"
        ...
        let body: Parameters = ["date":"2019/3"]
        
        Alamofire.request(URL, method: .get, parameters: body, headers: header)
            .responseData { response in
                switch response.result {
                case .success:
                    if let value = response.result.value {
